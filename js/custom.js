...
/* Principal Feedback Slider */
jQuery(document).ready(function () {
  var principalFeedback = jQuery('#principal-feedback-slider');
  if (!principalFeedback.length) return;
  principalFeedback.owlCarousel({
    items: 1,
    loop: true,
    autoplay: true,
  ...
  jQuery('.principal-prev').on('click', function () {
    principalFeedback.trigger('prev.owl.carousel');
  });
  jQuery('.principal-next').on('click', function () {
    principalFeedback.trigger('next.owl.carousel');
  });
});
/* Principal Feedback Slider End */
